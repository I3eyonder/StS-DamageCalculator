...
@SpireInitializer
class DmgCalculatorMod : PostInitializeSubscriber, PostRenderSubscriber, CardHoverSubscriber {
    ...
    override fun onCardHovered(card: AbstractCard) {
        logger.info("Card $card is hovering")
        hoveredCard = card
    }

    override fun onCardUnHovered() {
        logger.info("Card unhovered")
        hoveredCard = null
    }

    override fun receivePostRender(spriteBatch: SpriteBatch?) {
        spriteBatch?.let {
            CalculateIncomingDamageRenderer.render(it)
            CalculateOutgoingDamageRenderer.render(it, hoveredCard)
        }
    }
